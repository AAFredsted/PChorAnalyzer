...
public:
  ...
  AbstractComProjection(const AbstractComProjection &other)
      : AbstractProjection(other), // copy base
        channelName(other.channelName), typeName(other.typeName) {}
  ...
  virtual bool validateFunctionDecl(
...
public:
  Ireceive(const std::string &channelName, const std::string &typeName,
           std::shared_ptr<IndexExpr> indexNode)
      : AbstractComProjection(ProjectionType::IReceive, channelName, typeName),
        indexNode(indexNode) {}
  ...
  virtual std::string toString() const override {
    return std::format("?{}[{}]<{}>.", this->channelName,
                       this->indexNode->getArithmeticExprString(),
                       this->typeName);
  }

  void print() const override {
    std::print("?{}[{}]<{}>.", this->channelName,
               this->indexNode->getArithmeticExprString(), this->typeName);
  }
  ...
  size_t getChannelIndex() const override {
    if (indexNode->isExprLiteral()) {
      ...
      std::println("[PchorProjection] warning: called getChannelIndex from "
                   "Ireceive with non-literal index");
      return 1;
    }
  }
...
public:
  Isend(const std::string &channelName, const std::string &typeName,
        std::shared_ptr<IndexExpr> indexNode)
      : AbstractComProjection(ProjectionType::ISend, channelName, typeName),
        indexNode(indexNode) {}
...
public:
  ...
  public:
    ...
    AbstractProjection &operator*() const { return *current; }
    AbstractProjection *operator->() const { return current; }
  ...
  std::unique_ptr<ProjectionList> clone() const {
    ...
    if (head) {
      // Step 1: Deep copy via head's clone (recursively copies rest of list
      // too)
