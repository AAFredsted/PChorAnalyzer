...
void CAST_PchorASTVisitor::visit(const ForEachExpr &expr) {
  /*
    CAST_Mapping is independant of indeces, so we just skip it and go for the
    expression list contained within
  */
...
void Proj_PchorASTVisitor::visit(const ParticipantExpr &expr) {
  ...
  if (literal < lower || literal > upper) {
    throw std::runtime_error(
        std::format("Index expression {} evaluated to {}, which is not within "
                    "the range of [{}, {}].",
                    indexExpr->toString(), literal, baseIndex->getLower(),
                    baseIndex->getUpper()));
  }
  ...
  if (this->isSender) {
    this->ctx->addLiteralProjection(
        key,
        std::make_unique<Psend>(this->currentChannelName, this->currentDataType,
                                this->channelIndex),
        lower, upper);
  } else {
    this->ctx->addLiteralProjection(
        key,
        std::make_unique<Preceive>(this->currentChannelName,
                                   this->currentDataType, this->channelIndex),
        lower, upper);
  }
}
...
void Proj_PchorASTVisitor::visit(const ForEachExpr &expr) {
  ...
  /*
    ...
    1. iterate over indeces from min to max defined in IterExpr and set that
    index in visitor, then call visit on child
    2. if end index is abstract, break expression down into equivalence classes
    if feasable (requires inductive proof for this property!)
  */
  ...
  if (baseIndex->getUpper() == std::numeric_limits<size_t>::max()) {
    ...
    // due to previous check, we know that max is not max, so we can check for
    // one above ! to stay on the safe side however, we project in the following
    // way
    while (true) {
...
void insertFullPattern(FullIter &iter,
  ...
  */
  ...
  default:
    throw std::runtime_error(
        std::format("[Proj_Visitor] The iteration pattern ({}: I) only allows "
                    "expressions of type (i|n-i). Found {}",
...
void insertMaxExPattern(MaxExcludingIter &iter,
                        ...
                        const std::string &identifier) {
  /*
    Inserting a communication expression (Isend or Ireceive) into
    MaxExcludingIter is defined for the following cases: case 1: i maps to
    forward, which is a, evenAD and unevenAB case 2: n-i+l maps to backward,
    which is b, evenBC and unevenAB case 3: i+1 maps to forwardPlus, which is c,
    evenBC and unevenCD case 4: n-i+l-1 maps to backwardMinus, which is d,
    evenAD and unevenCD
  ...
  */
  ...
  default:
    throw std::runtime_error(std::format(
        "[Proj_Visitor] The iteration pattern ({} < max(I)) only allows "
        "expressions of type (i|n-i+l|i+1|n-i+l-1). Found {}",
        identifier, participantIndex->toString()));
  }
}
...
void insertMinExPattern(MinExcludingIter &iter,
  ...
  /*
    Inserting a communication expression (Isend or Ireceive) into
    MinExcludingIter is defined for the following cases: case 1: i maps to
    forward, which is a, evenAD and unevenAB case 2: n-i+l maps to backward,
    which is b, evenBC and unevenAB case 3: i-1 maps to forwardPlus, which is c,
    evenBC and unevenCD case 4: n-i+l+1 maps to backwardPlus, which is d, evenAD
    and unevenCD
...
Proj_PchorASTVisitor::getCasesFull(const std::shared_ptr<ExprList> &expr,
  ...
  /*
    ...
    We use a symbol table to store one such pattern for each unique participant
    group
  */
  ...
  for (const auto &com : *expr) {
    ...
    if (com->getExprType() != Expr::ComExpr) {
      throw std::runtime_error(std::format(
          "Only Com Allowed in unbounded foreach, found {}", com->toString()));
    }
    // convert pointer to descendent in class hierachy
    // ExprPchorASTNode->CommunicationExpr
    std::shared_ptr<CommunicationExpr> comExpr =
        std::dynamic_pointer_cast<CommunicationExpr>(com);
    ...
    // To do thiw, we need the exprList, the sender/reciever, the channelname,
    // the datatype and the identifier in the loop (to ensure coherence with
    // loop)
    const auto channel = comExpr->getChannel();
    const std::string &channelName = channel->getBaseParticipant()->getName();
    const auto dataType = comExpr->getDataType();

    // We do not allow for index expressions only containing literals in sender,
    // receiver or channel
    if (sender->getIndex()->isExprLiteral() ||
        receiver->getIndex()->isExprLiteral() ||
        channel->getIndex()->isExprLiteral()) {
      throw std::runtime_error(std::format(
          "[Proj_Visitor] Equivalence Class projection requires indeces to be "
          "expressions. Found {} and {}",
          sender->toString(), receiver->toString(), channel->toString()));
    }
    ...
    insertFullPattern<Isend>(BasePattern.at(senderName), sender, channel,
                             dataType, i);
    ...
    insertFullPattern<Ireceive>(BasePattern.at(receiverName), receiver, channel,
                                dataType, i);
  }
  return BasePattern;
}
...
Proj_PchorASTVisitor::getCasesMaxEx(const std::shared_ptr<ExprList> &expr,
                                    const std::string &i) const {
  /*
  ...
  */
  ...
  for (const auto &com : *expr) {
    ...
    insertMaxExPattern<Isend>(BasePattern.at(senderName), sender, channel,
                              dataType, i);
    ...
    insertMaxExPattern<Ireceive>(BasePattern.at(receiverName), receiver,
                                 channel, dataType, i);
  }
  return BasePattern;
}
std::unordered_map<std::string, MinExcludingIter>
Proj_PchorASTVisitor::getCasesMinEx(const std::shared_ptr<ExprList> &expr,
                                    const std::string &i) const {
  /*
  ...
  */
  ...
  for (const auto &com : *expr) {
    ...
    insertMinExPattern<Isend>(BasePattern.at(senderName), sender, channel,
                              dataType, i);
    ...
    insertMinExPattern<Ireceive>(BasePattern.at(receiverName), receiver,
                                 channel, dataType, i);
  }
  return BasePattern;
}
void Proj_PchorASTVisitor::addEquivalenceClassesFull(
    std::unordered_map<std::string, FullIter> &baseCases,
    [[maybe_unused]] const std::string &identifier,
    [[maybe_unused]] const std::string &indexName, size_t l, size_t n) {
  /*
    ...
    else we also need to care about b and a+b, where the following pattern
    appears

    for i: I = [l,n] and K=|I| and c=rounddown((K-l)/2)+l, the following
    patterns emerge for all n and l<n: a.b : [l,c) for K|2 and K!|2 a+b : [c,c]
    for K!|2 b.a : [c,n] K|2 and (c,n] for K!|2

    We turn the above into equivalence classes and add them to our key system

    In total, three distinct patterns are possible, against which must be
    checked for all.
  */
  ...
  for (const auto &[name, bases] : baseCases) {
    ...
    Range r1 = Range{Bound{RangeSymbol::L, true, true},
                     Bound{RangeSymbol::C, false, false}};
    ...
    // a+b : [c,c] for K!|2
    Range r2 = Range{Bound{RangeSymbol::C, true, true},
                     Bound{RangeSymbol::C, true, false}};
    ...
    Range r3e = Range{Bound{RangeSymbol::C, true, true},
                      Bound{RangeSymbol::N, true, false}};
    ParticipantKey key3e{name, r3e, EvenCase::Even};
    Range r3o = Range{Bound{RangeSymbol::C, false, true},
                      Bound{RangeSymbol::N, true, false}};
...
void Proj_PchorASTVisitor::addEquivalenceClassesMaxEx(
    std::unordered_map<std::string, MaxExcludingIter> &baseCases,
    [[maybe_unused]] const std::string &identifier,
    [[maybe_unused]] const std::string &indexName, size_t l, size_t n) {
  /*
    for every MaxExcludingIter, we run the following assesment:
    if(backward.empty() and backwardMinus.empty()) we only need to care about
    forward and forwardPlus case This logic has not been added, but will be at
    some future date
    ...
    else we also need to care about  a, b, c, d, and all overlaps (a+b, c+d |
    a+d, b+c)

    for i: I = [l,n] and K=|I| and k=rounddown((K-l)/2)-l, the following
    patterns emerge for all n and l<n:
    ...
    In total, seven distinct patterns are possible, against which must be
    checked for all.
  */
  for (const auto &[name, bases] : baseCases) {
    ...
    ParticipantKey key1{name,
                        Range{Bound{RangeSymbol::L, true, true},
                              Bound{RangeSymbol::L, true, false}},
                        EvenCase::Both};
    ...
    ParticipantKey key2e{name,
                         Range{Bound{RangeSymbol::L, false, true},
                               Bound{RangeSymbol::lC, false, false}},
                         EvenCase::Even};
    ParticipantKey key2o{name,
                         Range{Bound{RangeSymbol::L, false, true},
                               Bound{RangeSymbol::lC, true, false}},
                         EvenCase::Odd};
    ...
    ParticipantKey key3{name,
                        Range{Bound{RangeSymbol::C, true, true},
                              Bound{RangeSymbol::C, true, false}},
                        EvenCase::Odd};
    ...
    ParticipantKey key4{name,
                        Range{Bound{RangeSymbol::lC, true, true},
                              Bound{RangeSymbol::lC, true, false}},
                        EvenCase::Even};
    ...
    ParticipantKey key6{name,
                        Range{Bound{RangeSymbol::C, false, true},
                              Bound{RangeSymbol::N, false, false}},
                        EvenCase::Both};
    ...
    ParticipantKey key7{name,
                        Range{Bound{RangeSymbol::N, true, true},
                              Bound{RangeSymbol::N, true, false}},
                        EvenCase::Both};
...
void Proj_PchorASTVisitor::addEquivalenceClassesMinEx(
    std::unordered_map<std::string, MinExcludingIter> &baseCases,
    [[maybe_unused]] const std::string &identifier,
    [[maybe_unused]] const std::string &indexName, size_t l, size_t n) {
  /*
    for every MinExcludingIter, we run the following assesment:
    if(backward.empty() and backwardPlus.empty()) we only need to care about
    forward and forwardPlus case This logic has not been added, but will be at
    some future date
