...
clang::FunctionDecl *
CASTValidator::validateFuncDecl(std::shared_ptr<CASTMapping> CASTMap,
  ...
  for (const auto &projection : projections) {
    ...
    if (funcDecl == nullptr) {
      funcDecl = const_cast<clang::FunctionDecl *>(currentFuncDecl);
...
bool CASTValidator::validateProjection(
  ...
  for (const auto &[groupname, participantGroup] : *projectionMap) {
    for (const auto &[participantName, projections] : participantGroup) {
      ...
      const auto *record =
          CASTmap->getMapping<const clang::Decl *>(participantName.name);
      const auto *castRecord = llvm::dyn_cast<clang::CXXRecordDecl>(record);

      if (!castRecord) {
        throw std::runtime_error(
            std::format("Participant {} did not map to a record in the "
                        "CASTmapping. Instead, mapped to:  {}\n",
      ...
      for (auto ritr = methods.rbegin(); ritr != methods.rend(); ++ritr) {
        ...
        bool successFullMapping = projectionNode->validateFunctionDecl(
            Context, CASTmap, itr, end, nestedFunctionNext);

        if (itr != end) {
          llvm::errs() << std::format(
              "Warning: Not all statements in {} consumed "
        ...
        if (successFullMapping) {
          ...
          successfullValidations[funcName].push_back(
              participantName.toString());
          break;
        } else {
