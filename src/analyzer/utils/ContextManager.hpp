...
/*
Enum Classes
The equivalence class projections are valid over a finite number of ranges over
I=[l,n], which can be identified using K=|l| and c=lower((k-l)/2)+l.
Furthermore, we differentiate between eq classes that are valid for even and odd
K. Thereby, we get the following
*/

enum class EvenCase : u_int8_t { Both, Even, Odd };
...
*/
enum class RangeSymbol : u_int8_t { L = 0, lC = 1, C = 2, N = 3 };
...
struct Bound {
  ...
  Bound(RangeSymbol symbol, bool inclusive, bool lhs)
      : symbol(symbol), inclusive(inclusive), lhs(lhs) {}

  bool isMinInclusive() { return lhs && inclusive && symbol == RangeSymbol::L; }
  bool isMaxInclusive() {
...
struct Range {
  ...
  explicit Range(const Bound &lower, const Bound &upper)
  ...
  bool isMinInclusive() { return lower.isMinInclusive(); }
  bool isMaxInclusive() { return upper.isMaxInclusive(); }
  ...
  Range intersection(const Range &other) const {
    if (!overlaps(other)) {
      throw std::runtime_error(std::format("Ranges do not overlap: {} {}",
                                           this->toString(), other.toString()));
    }
    // i know it looks weird, but I don't want to write default constructors,
    // and the compiler complains if write it differently
    return Range{(lower <= other.lower) ? other.lower : lower,
  ...
  Range leftDifference(const Range &other) const {
    ...
    if (lower < other.lower) {
      Bound newUpper =
          Bound(other.lower.symbol, !other.lower.inclusive, !other.lower.lhs);
      return Range(lower, newUpper);
    }

    throw std::runtime_error(std::format(
        "Invalid Ranges Received, no right difference for {} without {}",
        this->toString(), other.toString()));
  }
...
/*
We use Variant to have a relatively simple way to deal with multiple keys in one
set.
*/
struct ParticipantKey {
  ...
  ParticipantKey(const std::string &name, size_t idx)
      : name(name), index(std::make_unique<size_t>(idx)), range(nullptr),
        even(nullptr) {}

  // Name + range + even
  ParticipantKey(const std::string &name, const Range &rng, EvenCase ev)
      : name(name), index(nullptr), range(std::make_unique<Range>(rng)),
        even(std::make_unique<EvenCase>(ev)) {}

  // Name + index + range + even
  ParticipantKey(const std::string &name, size_t idx, const Range &rng,
                 EvenCase ev)
      : name(name), index(std::make_unique<size_t>(idx)),
        range(std::make_unique<Range>(rng)),
        even(std::make_unique<EvenCase>(ev)) {}
  ...
  Range intersection(const Range &other) const {
    if (!range) {
      throw std::runtime_error(
          std::format("intersection not defined for key {}", this->toString()));
    }
  ...
  bool operator==(const ParticipantKey &other) const {
    if (name != other.name)
      return false;
    if (static_cast<bool>(index) != static_cast<bool>(other.index))
      return false;
    if (index && other.index && *index != *other.index)
      return false;
    if (static_cast<bool>(range) != static_cast<bool>(other.range))
      return false;
    if (range && other.range && !(*range == *other.range))
      return false;
    if (static_cast<bool>(even) != static_cast<bool>(other.even))
      return false;
    if (even && other.even && *even != *other.even)
      return false;
    return true;
  }
};
...
public:
  ...
  void addParticipantGroup(const std::string &name) {
    if (!projectionMap.contains(name)) {
      projectionMap.emplace(
          name,
          std::unordered_map<ParticipantKey, std::shared_ptr<ProjectionList>,
                             ParticipantKeyHash>{});
    }
  }
  ...
  bool hasParticipant(const ParticipantKey &key) const {
    return projectionMap.contains(key.name) &&
           projectionMap.at(key.name).contains(key);
  }
  ...
  void addParticipant(const ParticipantKey &participantName) {
    ...
    projectionMap[participantName.name].emplace(
        participantName, std::make_shared<ProjectionList>());
  }
  ...
  void appendCloneProjection(const ParticipantKey &key,
                             std::shared_ptr<ProjectionList> proj) {
  ...
  bool hasMathingRangeToLiteral(const ParticipantKey &key, size_t l,
                                size_t n) const {
    size_t i = *(key.index);
  ...
  void addLiteralProjection(const ParticipantKey &key,
    ...
    // case 3: there exists a P{range evencase}, which we need to have our
    // literal number map to
      ...
      std::vector<std::pair<ParticipantKey, std::shared_ptr<ProjectionList>>>
          keysToInsert;
      size_t i = *(key.index);
      ...
      /*
        i = l
          range [n,l] is turned into (l, n] with new projection P[l][l,l]
        existing (that is all)

        i \in (l, n)
            P[i]range is generated for all ranges for P
        i = n
          range [n,l] is turned into [l, n) with new projection P[n][n,n]
        existing (that is all)
      */
      //(l,n)
      Range lnRange{Bound{RangeSymbol::L, false, true},
      ...
      for (const auto &[iKey, iProj] : projectionMap.at(key.name)) {
        ...
        if (i == l && !iKey.index && iKey.isMinInclusive()) {
          ...
          // if range was not [l,l], we generate (l,k for whatever was there
          // before !
          if (iKey.range->isRightDifference(ll)) {
            ...
            std::shared_ptr<ProjectionList> rightDiffProj =
                std::make_shared<ProjectionList>();
        ...
        // case 2: max
          ...
          // if range was not [n,n], we generate k,n) for whatever  k was there
          // before !
          if (iKey.range->isLeftDifference(nn)) {
            ...
            std::shared_ptr<ProjectionList> leftDiffProj =
                std::make_shared<ProjectionList>();
        ...
        // case 3: (l,n)
          ...
          // in this case, we take any ranges witin (l,n) and add our own twist
          // (no need for differencing, just straight copying across an entire
          // range)
          Range range = lnRange.intersection(*(iKey.range));
          ParticipantKey lnKey{iKey.name, i, range, *(iKey.even)};
          std::shared_ptr<ProjectionList> lnProj =
              std::make_shared<ProjectionList>();
    ...
    // final case, we add ourselves as no ranged participants were identified to
    // map onto
    else {
      // we have no ranged
  ...
  // missing handling of literal case. Once this is done, we are completely done
  // and can debug ! only to be used in ranged loops with no upper bound (i.e we
  // only call this function from those cases)
  /*
  Bug: currently,this function does not handle the cases of even|odd|both for
  key and rKey, it simply uses the even attribute from rKey.
  */
  void appendCloneRangedProjection(const ParticipantKey &rKey,
                                   std::shared_ptr<ProjectionList> rProj,
                                   size_t l, size_t n) {
    ...
    Range limitedRange = Range{Bound{RangeSymbol::L, false, true},
                               Bound{RangeSymbol::N, false, false}};
    ...
    for (const auto &[key, proj] : projectionMap[rKey.name]) {
      ...
      if (key.index && !key.range) {
        ...
        if (index == l && rKey.isMinInclusive()) {
          ...
          Range ll = Range{Bound{RangeSymbol::L, true, true},
                           Bound{RangeSymbol::L, true, false}};
          ...
          Range nn = Range{Bound{RangeSymbol::N, true, true},
                           Bound{RangeSymbol::N, true, false}};
          ...
          if (limitedRange.isLeftDifference(pRange)) {
            ParticipantKey LKey{key.name, *(key.index),
                                limitedRange.leftDifference(pRange), pEven};
            keysToInsert.emplace_back(LKey, proj);
          }

          // make right
          if (limitedRange.isRightDifference(pRange)) {
            ParticipantKey RKey{key.name, *(key.index),
                                limitedRange.rightDifference(pRange), pEven};
      ...
      } else if (key.index && key.range &&
                 (*(key.even) == EvenCase::Both ||
                  *(rKey.even) == EvenCase::Both ||
                  *(key.even) == *(rKey.even))) {
        // final case to solve
        size_t i = *(key.index);
      ...
      } else if (key.range && (*(key.even) == EvenCase::Both ||
                               *(rKey.even) == EvenCase::Both ||
                               *(key.even) == *(rKey.even))) {
        const Range &L = *key.range;
        const Range &R = *rKey.range;
  ...
  void printEfficient() const {
    ...
    for (const auto &[elem, value] : projectionMap) {
      std::unordered_map<const void *, std::vector<const ParticipantKey *>>
          reverseMap;
      for (const auto &[key, projection] : value) {
