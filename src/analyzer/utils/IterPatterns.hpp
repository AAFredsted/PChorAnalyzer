...
struct FullIter {
  ...
  */
  ...
  void addForward(const std::string &channelName, const std::string &typeName,
                  std::shared_ptr<IndexExpr> indexNode) {

    this->forward->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
    this->unevenOverlapAB->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
  }
  // adds expression to basic type b and a+b
  template <typename ComType>
    requires std::derived_from<ComType, AbstractProjection>
  void addBackward(const std::string &channelName, const std::string &typeName,
                   std::shared_ptr<IndexExpr> indexNode) {
    this->backward->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
    this->unevenOverlapAB->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
  }
...
struct MaxExcludingIter {
  ...
  */
  ...
  template <typename ComType>
    requires std::derived_from<ComType, AbstractProjection>
  void addForwardPlus(const std::string &channelName,
    ...
    */
    this->forwardPlus->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
    this->evenOverlapBC->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
    this->unevenOverlapCD->appendBack(
        std::make_unique<ComType>(channelName, typeName, indexNode));
  }
  template <typename ComType>
    requires std::derived_from<ComType, AbstractProjection>
  void addBackwardMinus(const std::string &channelName,
                        const std::string &typeName,
                        std::shared_ptr<IndexExpr> indexNode) {
...
struct MinExcludingIter {
  ...
  template <typename ComType>
    requires std::derived_from<ComType, AbstractProjection>
  void addForwardMinus(const std::string &channelName,
                       const std::string &typeName,
                       std::shared_ptr<IndexExpr> indexNode) {
  ...
  template <typename ComType>
    requires std::derived_from<ComType, AbstractProjection>
  void addBackwardPlus(const std::string &channelName,
                       const std::string &typeName,
                       std::shared_ptr<IndexExpr> indexNode) {
